...
def get_or_create_channel(channel_data):
    attrs = {
...
def update_or_create_entry(entry_data, channel_id):
    attrs = {
        ...
        'content': entry_data.content[0].value,
        'content_type': entry_data.content[0].type,
...
def update_channels():
    ...
    for c in found_channels:
        channel_data = fetch_channel(c.link)
        # Only process updated channels.
        if to_datetime(channel_data.updated_parsed) > c.publication_updated_datetime:
            create_entries(channel_data.entries, c.id)
            update_channel(channel_data, c)
